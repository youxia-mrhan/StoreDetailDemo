 ...
 * 扩展 CommonNavigator
 */
public class CommonNavigatorEx extends FrameLayout implements IPagerNavigator, NavigatorHelperEx.OnNavigatorExScrollListener {
    ...
    private NavigatorHelperEx mNavigatorHelper;
    ...
    public CommonNavigatorEx(Context context) {
        super(context);
        mNavigatorHelper = new NavigatorHelperEx();
    ...
    @Override
    public void onSelected(int index, int totalCount, int previousIndex) {
        if (mTitleContainer == null) {
            return;
        }
        View v = mTitleContainer.getChildAt(index);
        if (v instanceof IPagerTitleView) {
            ((IMeasurablePagerTitleViewEx) v).onSelected(index, totalCount, previousIndex, getPagerTitleView(previousIndex));
        }
        if (!mAdjustMode && !mFollowTouch && mScrollView != null && mPositionDataList.size() > 0) {
            int currentIndex = Math.min(mPositionDataList.size() - 1, index);
            PositionData current = mPositionDataList.get(currentIndex);
